...
app.post("/api/v1/students", async (req, res) => {
  await connection.execute(
    `INSERT INTO students VALUES (?, ?, ?, FALSE)`,
    [null,req.body.namee, req.body.generation]
  );
...
app.put("/api/v1/students/:id", async (req, res) => {
  await connection.execute(
    `UPDATE students SET namee = ?, generation = ?, present = ? WHERE id = ?`,
    [req.body.namee, req.body.generation, req.body.present, req.params.id]
  );
